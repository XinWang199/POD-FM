...
def plot_compare_scalar_new(
    ...
    # GT + sensors
    im0 = ax[0].imshow(gt, cmap=flow_cmap, vmin=gt_recon_vmin, vmax=gt_recon_vmax)
    if show_points:
        ys, xs = np.where(mk > 0.5)
        ax[0].scatter(xs, ys, s=18, marker='*', facecolors='none',
                     edgecolors='k', linewidths=0.9)
    ax[0].set_title("GT + sensors")
    ax[0].axis("off")
    # 调整颜色棒大小：shrink参数控制高度，aspect参数控制宽度
    fig.colorbar(im0, ax=ax[0], shrink=0.5, aspect=20)

    # Recon
    im1 = ax[1].imshow(rc, cmap=flow_cmap, vmin=gt_recon_vmin, vmax=gt_recon_vmax)
    ax[1].set_title("Recon")
    ax[1].axis("off")
    fig.colorbar(im1, ax=ax[1], shrink=0.5, aspect=20)

    # |err|
    im2 = ax[2].imshow(abs_err, cmap=err_cmap, vmin=err_vmin, vmax=err_vmax)
    ax[2].set_title("|err|")
    ax[2].axis("off")
    fig.colorbar(im2, ax=ax[2], shrink=0.5, aspect=20)
    # c2.set_label("abs error", rotation=270, labelpad=10)

    # 去掉整个图的标题
    # fig.suptitle(f"Frame t={t_idx}", fontsize=12)  # 注释掉这行
...
def main():
    ...
    ap.add_argument("--first_n", type=int, default=5000)
    ap.add_argument("--seq_len", type=int, default=1)
    ap.add_argument("--split_dir", type=str, default="runs/Cylinder_3rank_1e-3/split_seq1_seed0_r0.8",
                    help="训练脚本生成的固定划分目录（含 train_idx.npy / test_idx.npy）")
    # 观测生成
    ap.add_argument("--mask_ratio", type=float, default=0.00094, help="单帧 mask 比例")
    ...
    # 模型
    ap.add_argument("--ckpt", type=str, default='runs/Cylinder_3rank_1e-3/final.pt')
    ...
    # POD
    ap.add_argument("--pod_npz", type=str, default='runs/Cylinder_3rank_1e-3/pod_basis_seq1_r3.npz', help="POD 基与均值 npz（训练端生成）")
    ap.add_argument("--pod_lam", type=float, default=1e-3, help="POD LS 正则 λ")
    ap.add_argument("--pod_dynamic_r", default=False)
    ap.add_argument("--pod_alpha", type=float, default=1.0)
    # 采样
    ap.add_argument("--steps", type=int, default=100)
    ap.add_argument("--solver", type=str, default="euler", choices=["heun","euler"])
    ap.add_argument("--hard_dc", default=True, help="是否每步做硬一致性投影")
    ap.add_argument("--batch_size", type=int, default=64)
    ap.add_argument("--limit_batches", type=int, default=24)
    ...
    # 其他
    ap.add_argument("--norm_path", type=str, default="runs/Cylinder_3rank_1e-3/flow_mean_std.npy", help="训练时保存的 flow_mean_std.npy（推荐传入）")
    ap.add_argument("--device", type=str, default="cuda:0")
    ap.add_argument("--seed", type=int, default=2025)
    ap.add_argument("--out_dir", type=str, default='runs/Cylinder_3rank_1e-3')
    ap.add_argument("--fixed_sampling_points", type=str, default="fixed_sampling_points_cylinder.npy", 
            help="fixed_sampling_points_cylinder.npy预生成的固定采样点文件路径")
    ...
    train_idx, test_idx = load_split(args.split_dir)
    ...
    if args.norm_path and os.path.isfile(args.norm_path):
        ...
        # 若未传入，尽量用训练子集估计
        mean,std = float(arr[train_idx].mean()), float(arr[train_idx].std())
        ...
        with torch.no_grad():
            ...
            # [NEW] 添加相对噪声到观测数据
                ...
                while saved < args.save_n and saved < x1.size(0):
                    ...
                    out_png = os.path.join(args.out_dir, f"noise_std_{args.obs_noise_std}/case{bi:03d}_{saved:02d}_t{t_show}.png")
