...
def main():
    ...
    ap.add_argument("--first_n", type=int, default=10000, help="取前 N 帧（<=0 则全量）")
    ap.add_argument("--seq_len", type=int, default=1, help="=1 为 2D，>1 为窗口")
    ap.add_argument("--test_ratio", type=float, default=0.3, help="测试集比例")
    ...
    ap.add_argument("--mask_ratio_min", type=float, default=0.004)
    ap.add_argument("--mask_ratio_max", type=float, default=0.048)
    ...
    ap.add_argument("--resume", type=str, default="runs/Pipe_8rank_1e-1/best.pt")
    # TensorBoard 配置
    ...
    # POD设置
    ap.add_argument("--pod_npz", type=str, default="runs/Pipe_8rank_1e-1/pod_basis_seq1_r8.npz",
    ...
    train_idx = np.arange(base_total, dtype=np.int64)
    test_idx = train_idx.copy()

    print(f"[Data] Using all {len(train_idx)} samples for both training and testing")

    # 记录数据集信息到 TensorBoard
    writer.add_text("dataset/info", f"Total samples: {N}, Using all data for train/test: {len(train_idx)}", 0)
    ...
    # ========== POD基处理：优先使用已存在的POD基 ==========
    pod_basis_path = os.path.join(args.out_dir, f"pod_basis_seq{args.seq_len}_r{8}.npz")
    if os.path.exists(pod_basis_path):
        ...
        X, H, W, C = build_snapshots_from_train(arr, train_idx, seq_len=args.seq_len, mean=tr_mean, std=tr_std)
        x_mean = X.mean(dim=0)                      # (D,)
        Xc = X - x_mean[None, :]

        # 计算 U
        U = compute_pod_basis_torch(Xc, r=8, device=device)  # (D,r)
