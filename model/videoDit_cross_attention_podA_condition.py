...
[CHG] 需求落实：
  - 不再把 Xpod 作为偏置或时间调制；
  - 外部仅传入 POD 系数 a_hat，与 y_obs 一起作为 **Cross-Attention 的条件**；
  - forward I/O 基本保持：仍然保留 xpod 形参以兼容，但 **忽略** xpod；新增可选 a_hat 形参；
  - 输出不变。

同时提供：
  - [NEW] pod_coeffs_from_obs_singleframe(...) —— 单帧场景下，从 (y_obs, mask1) 直接求 a_hat（外部可调用）。
"""
...
def pod_coeffs_from_obs_singleframe(
    ...
    """
    根据 (y_obs, mask1) 直接求解 POD 系数（不重建场），适用于单帧样本。
    返回 a_hat: (B, r_max)。
    """
...
# ============================== #
#  新版：a_hat + y_obs 作为 Cross 条件  #
# ============================== #
class VideoDiT_FM_XAttn(nn.Module):
    """
    条件化的时空 DiT：
      - 继续使用 y_obs -> SensorTokenizer 的稀疏 token 作为 Cross-Attn 条件；
      - [NEW] 外部传入的 POD 系数 a_hat（B,r），经过 Linear(r,D) 投影为一个 **全局系数 token**，
              与 y_obs 的稀疏 token **在 token 维上拼接**，共同作为 Cross-Attn 的 K/V；
      - 不再使用 xpod；时间条件 c 仅由 t_embedder 提供（不再把 a_hat 加到时间向量）。
    """
    def __init__(self,
                 video_size=(1, 64, 64),
                 patch_size=(1, 4, 4),
    ...
    def forward(self,
        ...
        # --- [修改后] 将 a_hat 映射并拼接到 Cross-Attn 条件序列 ---
        if a_hat is not None:
            # 检查模型在初始化时是否已配置好接收 a_hat
