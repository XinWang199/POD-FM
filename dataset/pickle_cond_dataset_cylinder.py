...
class PickleCondDataset(Dataset):
    ...
    def __init__(
        ...
        # ROI
        roi_type: Optional[str] = None,            # None | "box" | "circle" | "polygon"
        roi_box: Optional[Tuple[int,int,int,int]] = None,
        roi_circle: Optional[Tuple[float,float,float]] = None,
        roi_polygon: Optional[List[Tuple[float, float]]] = None, # [(y1,x1), (y2,x2), ...]
        ...
        # ROI
        self.roi_mask2d = self._build_roi_mask2d(self.H, self.W, roi_type, roi_box, roi_circle, roi_polygon)
    ...
    @staticmethod
    def _build_roi_mask2d(H, W, roi_type, roi_box, roi_circle, roi_polygon):
        if roi_type is None: return np.ones((H,W), bool)
        xx, yy = np.meshgrid(np.arange(W), np.arange(H))

        if roi_type == "box":
            assert roi_box is not None, "roi_type='box' 需要 roi_box 参数"
            h0,h1,w0,w1 = roi_box; m = np.zeros((H,W), bool); m[h0:h1, w0:w1] = True; return m
        
        if roi_type == "circle":
            assert roi_circle is not None, "roi_type='circle' 需要 roi_circle 参数"
            cy,cx,r = roi_circle; return ((yy-cy)**2 + (xx-cx)**2 <= r**2)

        # --- 新增的任意多边形区域逻辑 ---
        if roi_type == "polygon":
            assert roi_polygon is not None and len(roi_polygon) >= 3, "roi_type='polygon' 需要 roi_polygon 参数，且至少包含3个顶点"
            
            # matplotlib.path 需要 (x, y) 格式的顶点
            # 我们的输入是 (y, x)，所以需要转换一下
            polygon_vertices_xy = [(x, y) for y, x in roi_polygon]
            
            # 创建一个 Path 对象
            path = MplPath(polygon_vertices_xy)
            
            # 创建网格中所有点的坐标列表
            points = np.vstack((xx.ravel(), yy.ravel())).T
            
            # 使用 contains_points 高效判断所有点是否在多边形内
            mask = path.contains_points(points)
            
            # 将一维的 mask 结果重塑为二维的 (H, W) 形状
