...
def main():
    ...
    ap.add_argument("--roi_type", type=str, default="none", choices=["none","box","circle"])
    ...
    ap.add_argument("--resume", type=str, default="runs/Pipe/best.pt", help="恢复训练的检查点路径")
    # TensorBoard 配置
    ...
    test_idx = train_idx.copy()  # 测试也使用同样的数据
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % 10 == 0:
            ...
            if improved:
                ...
                save_ckpt_with_step = lambda path, epoch, model, opt, ema, cfg, step: torch.save({
                    "epoch": epoch,
                    "global_step": step,
                ...
                save_ckpt_with_step(ckpt, epoch, model, opt, ema, cfg, global_step)
                print(f"[Save] {ckpt}")
            if epoch == 800:
