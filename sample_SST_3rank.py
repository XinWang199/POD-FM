...
# ---------------- 数据/工具 ----------------
@torch.no_grad()
def compute_metrics(x_gt, x_rec, mask, ocean_mask=None, eps: float = 1e-12):
    """
    x_* : (B, C, T, H, W) ；mask : (B, 1, T, H, W) ，float型 0/1
    ocean_mask: (H, W) 或 (1, 1, 1, H, W)，float型 0/1，1表示海洋（有效区域），0表示陆地
    返回：
    ...
    if ocean_mask is not None:
        if ocean_mask.dim() == 2:
            # (H, W) -> (1, 1, 1, H, W)
            ocean_mask = ocean_mask.float().unsqueeze(0).unsqueeze(0).unsqueeze(0)
        ocean_mask = ocean_mask.to(x_gt.device)
    else:
        ocean_mask = torch.ones(1, 1, 1, x_gt.shape[3], x_gt.shape[4], device=x_gt.device)
    
    # 只在海洋区域计算误差
    diff  = (x_rec - x_gt) * ocean_mask
    diff2 = diff ** 2
    abs_err = diff.abs()

    ocean_cnt = ocean_mask.sum().clamp(min=1).item() * x_gt.shape[0] * x_gt.shape[2]  # B * T * ocean_pixels
    mse_all = diff2.sum().item() / ocean_cnt
    
    # 观测点和非观测点（都在海洋区域内）
    obs_mask = mask * ocean_mask
    unobs_mask = (1.0 - mask) * ocean_mask
    obs_cnt    = obs_mask.sum().clamp(min=1).item()
    unobs_cnt  = unobs_mask.sum().clamp(min=1).item()
    mse_obs    = (diff2 * mask).sum().item() / obs_cnt
    mse_unobs  = (diff2 * (1.0 - mask)).sum().item() / unobs_cnt

    def rel_l2(m):
        m_ocean = m * ocean_mask
        num = torch.sqrt((diff.pow(2) * m_ocean).sum(dim=(1, 2, 3, 4)))
        den = torch.sqrt(((x_gt * m_ocean).pow(2)).sum(dim=(1, 2, 3, 4))).add_(eps)
    ...
    # 只取海洋区域的 gt 值
    gt_ocean = x_gt * ocean_mask
    gt_ocean_flat = gt_ocean[ocean_mask.expand_as(x_gt) > 0.5]
    gt_range = (gt_ocean_flat.max() - gt_ocean_flat.min()).item()
    nrmse_range = rmse_all / (gt_range + eps)
    gt_std = gt_ocean_flat.std().item()
    nrmse_std = rmse_all / (gt_std + eps)

    # === 误差统计（仅海洋区域） ===
    abs_err_ocean = abs_err[ocean_mask.expand_as(abs_err) > 0.5]
    err_stats = {
        "mean": abs_err_ocean.mean().item(),
        "std": abs_err_ocean.std().item(),
        "max": abs_err_ocean.max().item(),
        "min": abs_err_ocean.min().item(),
        "median": abs_err_ocean.median().item(),
        "p95": torch.quantile(abs_err_ocean.flatten(), 0.95).item(),
        "p99": torch.quantile(abs_err_ocean.flatten(), 0.99).item(),
    }
...
def main():
    ...
    # 观测/ROI
    ap.add_argument("--mask_ratio", type=float, default=0.000563)
    ...
    # 模型
    ap.add_argument("--ckpt", type=str, default='runs/SST_3rank_1e-1/final.pt')
    ...
    # POD
    ap.add_argument("--pod_npz", type=str, default='runs/SST_3rank_1e-1/pod_basis_seq1_r3.npz', help="POD 基与均值 npz（训练端生成）")
    ap.add_argument("--pod_lam", type=float, default=1e-1)
    ap.add_argument("--pod_dynamic_r", default=False, help="POD 是否动态截断秩")
    ...
    # 其他
    ap.add_argument("--norm_path", type=str, default="runs/SST_3rank_1e-1/flow_mean_std.npy", help="训练时保存的 flow_mean_std.npy（推荐传入）")
    ...
    # 创建海洋掩码（陆地区域值接近0）
    mean_map = np.mean(arr_test, axis=0).squeeze()  # (H, W)
    ocean_mask_np = (np.abs(mean_map) > 1e-6).astype(np.float32)
    ocean_mask = torch.from_numpy(ocean_mask_np)  # (H, W)
    print(f"[Ocean Mask] Ocean pixels: {ocean_mask_np.sum():.0f} / {H*W} ({100*ocean_mask_np.mean():.2f}%)")
    ...
    for noise_level in noise_levels_to_test:
        ...
        with torch.no_grad():
            ...
            # [NEW] 添加相对噪声到观测数据
                ...
                mse_all, mse_obs, mse_unobs, r_all, r_obs, r_unobs, nrmse_range, nrmse_std, err_stats = compute_metrics(x1, x_rec, mask1, ocean_mask=None) # ocean_mask)
                ...
                print(f"[Batch {bi}] time={dur:.2f}s  "
        ...
        print(f"Note: All metrics are computed on OCEAN regions only (excluding land).")
        print(f"\n--- MSE Metrics ---")
